...
| `panels` | array | Ordered list of panel boxes. MAY be empty. |
...
1. Resolve the sibling `.cbxy` for the open comic.
2. Enumerate page images from the comic in the application’s normal page order.
3. For each page image path `P`, open entry `stem(P).json` with the same directory prefix.
...
1. Write a `.cbxy` ZIP next to the comic, or to an explicitly chosen path.
2. Emit one `.json` entry per processed page.
3. Set `page` to the image’s path inside the comic.
4. Set `width` / `height` to the pixel size used when measuring boxes.
5. Store panels in intended reading order with normalized coordinates.
...
There is no required version field inside the ZIP.
